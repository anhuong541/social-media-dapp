...
contract SocialMediaV4 {
    ...
    function getAllStatuses() public view returns (string[][] memory) {
        ...
        for (uint256 i = 0; i <= lastStatusId[owner]; i++) {
            if (lastStatusId[owner] > 0) {
                totalUsers++;
            }
        }

        string[][] memory allStatuses = new string[][](totalUsers * 3);

        uint256 userIndex = 0;
        for (uint256 i = 0; i <= lastStatusId[owner]; i++) {
            if (lastStatusId[owner] > 0) {
                string[] memory userStatuses = new string[](
                    lastStatusId[owner] * 3
                );
                for (uint256 j = 0; j <= lastStatusId[owner]; j++) {
                    uint256 index = 3 * j;
                    userStatuses[index] = statuses[owner][j];
                    userStatuses[index + 1] = uintToStr(likes[owner][j]);
                    userStatuses[index + 2] = uintToStr(comments[j].length);
                }
                allStatuses[userIndex] = userStatuses;
                userIndex++;
            }
        }
